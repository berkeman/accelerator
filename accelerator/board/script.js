const parseANSI = (function () {
	...
	function parse_extended_colour(groups) {
		...
		if (colour_type === 2) {
			...
			// RGB is stored << 8 to not collide with the 256 colour stuff
			return (r << 24) | (g << 16) | ((b & 255) << 8);
			// @@ do the string right away?
		...
		for (const part of parts.slice(1)) {
			...
			if (ix < part.length) {
				if (settings.fg !== null || settings.bg !== null || settings.attr.size) {
					...
					el.appendChild(document.createTextNode(part.slice(ix)));
				}
			}
		}
	}
